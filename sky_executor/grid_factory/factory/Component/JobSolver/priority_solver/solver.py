...
from sky_executor.grid_factory.factory.Component.JobSolver.utils.op_priority_greedy import (
    priority_greedy,
)
...
# 暂时不接收新的job
# 当前使用的字段不对，operation的machine option字段已变更
...
class GreedyJobSolver(JobSolver):
    ...
    def plan(self, obs: dict) -> dict:
        ...
        if not self.initialized:
            ...
            for job in jobs:
                for op in job.ops:
                    if not op.machine_options and not op.machine_options_with_time:
                        print(
                            f"[ERROR] Job {job.job_id} Op {op.op_id} has NO machine_options!"
                        )
                    else:
                        print(
                            f"[JobSolver] Job {job.job_id} Op {op.op_id}: "
