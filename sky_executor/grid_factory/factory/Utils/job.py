...
def generate_jobs(job_config: JobConfig) -> List[Job]:
    ...
    if job_config.strategy == "random":
        ...
        for j, job_info in enumerate(job_config.custom_jobs):
            ops: List[Operation] = []
            for op_id, op_info in enumerate(job_info):
                options_with_time = op_info[0]  # [(machine_id, proc_time), ...]
                op_processing_time_config = op_info[2] if len(op_info) > 2 else None
                op = Operation(
                    job_id=j,
                    op_id=op_id,
                    machine_options=[mid for mid, _pt in options_with_time],
                    machine_options_with_time=options_with_time,
                    proc_time=op_info[1],
                    nominal_proc_time=op_info[1],
                    processing_time_config=op_processing_time_config,
                )
                ops.append(op)
            jobs.append(Job(job_id=j, ops=ops))
    return jobs
