...
class Logger:
    def __init__(self, terminal_output=False, file_output=True, log_path=config.LOG_DIR, name="mylog"):
        ...
        if file_output is True:
            ...
            if os.path.islink(log_dir) and not os.path.exists(log_dir):
                # 符号链接指向的目标不存在，先创建目标目录
                target = os.readlink(log_dir)
                os.makedirs(os.path.dirname(target) if not os.path.isabs(target) else target, exist_ok=True)
