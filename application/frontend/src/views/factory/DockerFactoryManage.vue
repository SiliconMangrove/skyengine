...
const archiveCurrentRun = () => {
  ...
  if (store.historyBuffer.length === 0) {
    return Promise.reject(new Error('本次运行没有收到状态帧，无法保存日志'));
  }
  archivePromise = analysisLog.finalizeFromStores(store, monitorStore, {
    ...currentExperimentMeta.value,
    factory_id: store.selectedFactoryId,
    algorithm: sim.algorithmString.value,
  }).finally(() => {
    archivePromise = null;
...
const handleStop = async () => {
  ...
  if (store.historyBuffer.length > 0) {
    try {
      const run = await archiveCurrentRun();
      ElMessage.success(`运行日志已保存：${run.total_steps} 步`);
    } catch (error) {
    ...
    eventHandlers: {
      state: (data) => {
        ...
        if (data.status === 'stopped' || data.status === 'finished') {
          ...
          archiveCurrentRun()
            .then((run) => ElMessage.success(`运行日志已保存：${run.total_steps} 步`))
            .catch((e) => {
              console.error('[DockerFactory] finalizeFromStores 失败:', e);
              ElMessage.warning(`运行日志保存失败：${e.message || e}`);
            });
          return;
        }
      },
...
onUnmounted(() => {
  if (sim.isRunningTest.value && store.historyBuffer.length > 0) {
    archiveCurrentRun().catch((e) => console.error('[DockerFactory] 卸载前归档失败:', e));
  }
  disconnectAllSSE();
  sim.cleanup(store);
});
