    ...
    <div class="fp-content">
      ...
      <template v-else-if="activeTab === 'agv'">
        <template v-if="selectedAgvIndex != null && agvCount > 0">
          ...
          <div class="fp-row">
            <span class="fp-k">任务阶段</span>
            <span class="fp-v">{{ selectedAgvPhaseLabel }}</span>
          </div>
          <div class="fp-row">
            <span class="fp-k">载货状态</span>
            <span class="fp-v">{{ selectedAgvLoaded ? '已载货' : '空载' }}</span>
          </div>
          <div v-if="selectedAgvHandlingRemaining > 0" class="fp-row">
            <span class="fp-k">操作剩余</span>
            <span class="fp-v">{{ selectedAgvHandlingRemaining }} 步</span>
          </div>
          <div class="fp-row">
...
const agvPhaseArr = computed(() => focusState.value?.grid_state?.agv_task_phase || [])
const agvHandlingRemainingArr = computed(() => focusState.value?.grid_state?.agv_handling_remaining || [])
const agvLoadedArr = computed(() => focusState.value?.grid_state?.agv_loaded || [])
...
const selectedAgvStatusLabel = computed(() => {
  ...
  return selectedAgvPhaseLabel.value
})
const selectedAgvPhase = computed(() => agvPhaseArr.value[selectedAgvIndex.value] || 'IDLE')
const selectedAgvPhaseLabel = computed(() => ({
  IDLE: '空闲',
  TO_PICKUP: '前往取料',
  PICKING: '取料中',
  TO_DROPOFF: '前往放料',
  DROPPING: '放料中',
}[selectedAgvPhase.value] || selectedAgvPhase.value))
const selectedAgvHandlingRemaining = computed(() =>
  Number(agvHandlingRemainingArr.value[selectedAgvIndex.value] || 0),
)
const selectedAgvLoaded = computed(() => agvLoadedArr.value[selectedAgvIndex.value] === true)
